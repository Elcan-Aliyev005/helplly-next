        ...
        <section
            ...
            <div className={'section-header text-center pr-8 shrink-0 xl:max-w[413px] 2xl:max-w-[420px] xl:text-left'}>
                ...
                <SectionSubTitle className={'!text-[13px] xl:ml-0 xl:max-w-[400px] xl:!text-[16px] xl:!leading-6 !leading-4 mt-2'}>
                    Vaxtınızı xərcləmədən ən uyğun xidməti seçin –
                    mövcud variantları müqayisə edin və sürətlə  peşəkarlarla əlaqə qurun.
                </SectionSubTitle>
            </div>
            <div className={"how-does-it-work-cards mt-[24px] xl:mt-[0px] w-full  grid grid-cols-1 items-start  md:grid-cols-2    gap-5"}>
                {servicesData.map((service,index)=>{
                    return(
                        <div key={index} className={'bg-[#F9F9F9] w-full p-4 flex flex-col justify-between group rounded-[16px] border border-transparent transition-all duration-1000 hover:border-[#108910] hover:bg-[#1089100A] min-h-[196px] lg:min-h-[256px]'}>
                            ...
                            <div>
                                <h3 className={'text-[20px] mb-2 leading-6 xl:leading-8 xl:text-[24px] font-semibold'}>{service.title}</h3>
                                <p className={'text-sm leading-5  max-w-[280px] lg:max-w-[347px]    text-[#212121AD]'}>{service.description}</p>
                            </div>
                        </div>
                    )
                })}
